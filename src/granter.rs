...
impl<'a> Granter for Password {
    fn grant_token(self) -> Result<Json, Custom<Json>> {
        ...
        if !account.verify_password(&self.credentials.password) {
            return Err(Custom(Status::Unauthorized, Json(json!("invalid credentials"))));
        }
        ...
        let refresh_token = RefreshTokenController
            ...
            .map_err(|e| match e {
                _ => Custom(Status::InternalServerError, Json(json!("unable to create refresh token"))),
            })?
            .refresh_token;
        ...
        let token = UserTokenController
            ...
            .map_err(|e| match e {
                _ => Custom(Status::InternalServerError, Json(json!("unable to create user token"))),
            })?
            .user_token;

        let jwt = jsonwebtoken::encode(&jsonwebtoken::Header::default(), &token, b"secret")
            .map_err(|e| match e {
                _ => Custom(Status::InternalServerError, Json(json!("error creating jsonwebtoken"))),
            })?;
